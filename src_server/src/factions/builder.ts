...
import Faction, { FactionTypes } from './faction';
...
class FactionBuilder {
	...
	constructor(name: string, government: boolean, type: FactionTypes) {
		this.reset(name, government, type);
	}
	...
	setWardrobe(position: PositionEx, wardrobe: Wardrobe) {
		this.faction.wardrobe = wardrobeCtrl.create(
			position,
			wardrobe,
			this.faction
		);
	...
	createWorkshop(position: PositionEx, products: any) {
		this.faction.workshop = workshop.create(
			position,
			products,
			this.faction
		);
	...
	createInventory(position: PositionEx, capacity: InventoryCapacity) {
		this.faction.inventory = inventory.create(
			position,
			capacity,
			this.faction
		);
	...
	createWarehouse(position: PositionEx, capacity: number) {
		this.faction.warehouse = warehouse.create(
			position,
			capacity,
			this.faction
		);
	...
	private reset(name: string, government: boolean, type: FactionTypes) {
		this.faction = new Faction(name, government, type);
	}
}
