...
class CuffsActions {
  ...
  private useHandcuffs(player: Player) {
    ...
    if (
      !player.entityIsNearby(target?.mp) ||
      this.inCuffs(target) ||
      target.mp.vehicle
    )
      return;
    ...
    if (!handcuffs) {
      return hud.showNotification(player, 'error', 'Nemate lisice kod sebe');
    }
  ...
  private useCableTie(player: Player) {
    ...
    if (!tie) {
      return hud.showNotification(player, 'error', 'Nemate vezice kod sebe');
    }
  ...
  private untiePlayer(player: Player) {
    ...
    if (knife?.name !== 'knife') {
      return hud.showNotification(player, 'error', 'Morate izvaditi nož');
    }
