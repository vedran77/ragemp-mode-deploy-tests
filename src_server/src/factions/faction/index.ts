...
export type FactionTypes = 'law' | 'gang' | 'mafia';

class Faction {
	public readonly name: string;

	public readonly type: FactionTypes;
	...
	getPlayers(atWork = false) {
		...
		return Array.from(members.keys()).flatMap((id) => {
			...
			return (
				!atWork || this.isAlreadyAtWork(player) ? player : []
			) as Player;
		});
	}
	...
	isAlreadyAtWork(player: Player) {
		return (
			...
			((!this.government ||
				player.mp.getOwnVariable('factionWork')) as boolean)
		);
	}
	...
	isLeader(player: Player, deputy = false) {
		return deputy
			? this.hasAccess(player, 'members')
			: this.hasAccess(player, 'leader');
	}
