...
class Fishing extends Service {
	...
	constructor() {
		super('fishing', { name: 'Ribolov', model: 68, color: 30 });
	...
	async onKeyPress(player: Player) {
		...
		if (equipment.getEquipment(player, 'hands')) {
			hud.showNotification(player, 'error', 'Sklonite predmet iz ruku');
			return;
		}

		if (this.isAlreadyFishing(player)) {
			return hud.showNotification(player, 'error', 'Već pecate');
		}
		...
		if (location.level > jobLevel) {
			return hud.showNotification(
				player,
				'error',
				`Ova lokacija zahteva ${location.level}-ti nivo`
			);
		}
		...
		if (!rod)
			return hud.showNotification(
				player,
				'error',
				'Nemaš štap za pecanje'
			);

		const bait = inventoryHelper.findItem(player.inventory, 'fish_bait');
		if (!bait) {
			return hud.showNotification(player, 'error', 'Nemaš mamac');
		}
	...
	private async useTools(
		...
		if (rodCapacity <= 0)
			...
			hud.showNotification(
				player,
				'info',
				`Vaš štap za pecanje će trajati još ${rodCapacity} puta`
			);
		}
	...
	private async takeFish(player: Player) {
		...
		hud.showNotification(
			player,
			'success',
			`Ulovio si ribu "${fishName}". Svaka čast! Trenutni nivo: ${level}, ${points} / ${
				levels[level - 1].points
			} XP`
		);
