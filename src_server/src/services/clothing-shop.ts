...
class ClothingShop extends Service {
	...
	constructor() {
		super('clothing_shop', { name: 'Prodavnica Odeće', model: 73, color: 24 });
	...
	private prepareForInventory(clothes: Clothes, data: Product, gender: string) {
	...
	private async buy(player: Player, product: Product, payment: PaymentType) {
		...
		const item = this.prepareForInventory(clothes, product, player.gender);
