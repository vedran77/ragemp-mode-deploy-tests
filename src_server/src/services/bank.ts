...
class Bank extends Service {
  constructor() {
    super('bank', { name: 'Banka', model: 500, color: 2, scale: 0.85 });
  }

  protected subscribeToEvents() {
    mp.events.subscribe({
      'Bank-KeyPress': this.onKeyPress.bind(this),
  ...
  private async createAccount(player: Player, custom?: string) {
    if (player.bankAccount) return mp.events.reject('Već imate račun');

    if (custom) {
      const isExists = await this.checkExists(custom);

      if (isExists) return mp.events.reject('Ovaj bankovni račun je zauzet');
  ...
  private async checkExists(account: string) {
    const count = await CharModel.findOne({
      bankAccount: account,
    }).countDocuments();
  ...
  private async cashOut(player: Player, sum: number) {
    if (!player.bankAccount) {
      return mp.events.reject('Morate imati bankovni račun, otvori te ga!');
    }
  ...
  private async transferMoney(player: Player, account: string, value: any) {
    ...
    if (!user) return mp.events.reject('Bankovni nalog nije registrovan');
