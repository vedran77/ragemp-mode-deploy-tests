...
class VehicleRent extends Service {
  ...
  private async rentVehicle(player: Player) {
    if (this.license && !player.hasLicense(this.license)) {
      return hud.showNotification(
        player,
        'error',
        'Nemate odgovarajuću dozvolu za ovo vozilo!'
      );
    }
  ...
  private onExitFromVehicle(player: Player, vehicle: VehicleMp) {
    if (vehicle.name === this.model) {
      ...
      rent.timer = setTimeout(
        this.terminateRent.bind(this, player),
        15 * 60 * 1000
      );
      hud.showNotification(
        player,
        'info',
        'Van vašeg vozila ste izašli, imate 15 minuta ili će se vozilo vratiti u rent centar!',
        false,
        4000
      );
    }
