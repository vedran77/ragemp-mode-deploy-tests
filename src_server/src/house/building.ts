...
class HouseBuilding {
	...
	enter(player: Player, house: House) {
		...
		if (player.mp.getOwnVariable('inCaptureZone')) {
			hud.showNotification(
				player,
				'error',
				'Ne možete ulaziti u kuću dok zauzimaš teritorija!',
				true
			);
			return;
		}
	...
	toggleBlip(house: House, player?: Player) {
		...
		else if (player) mp.blips.delete(player, `Kuća ${house.index}`);
	}

	private createInventory(
		position: PositionEx & { rotation: number },
		dimension = 0
	) {
	...
	private createPassage(position: PositionEx, dimension = 0) {
		...
		mp.markers.new(
			...
			{
				dimension,
				color: dimension ? [255, 255, 255, 120] : [60, 179, 113, 120],
