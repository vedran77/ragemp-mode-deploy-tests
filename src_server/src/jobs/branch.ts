...
abstract class JobBranch {
  ...
  protected setClothes(player: Player) {
    ...
    Object.entries(this.clothes[player.gender]).forEach(([name, item]) => {
      playerClothes.set(player, name as any, {
        style: item[0],
        color: item[1],
      });
    });
  ...
  protected createOrder(player: Player) {
    ...
    if (isNumber(worker.order))
      throw new SilentError('job order has been declared');
  ...
  protected async giveSalary(player: Player) {
    const payout = Math.round(random(1, 1.2) * this.salary);

    await money.change(player, 'cash', payout, `job salary | ${this.name}`);

    player.mp.outputChatBox(
      chatErrorMessages.custom('POSAO', `Zaradili ste: $${payout}`)
    );
  }
}
