...
class Handler extends Branch {
  ...
  protected createOrder(player: Player) {
    ...
    if (!vehicle || player.mp.vehicle.id !== vehicle.id) {
      hud.showNotification(player, 'info', 'Niste u poslovnom vozilu!', true);
      return;
    }

    attachments.add(vehicle, 'fork_container');
