...
class PlayerDocs {
  ...
  offer(player: Player, type: Document) {
    ...
    offers.showWithExpires(customer, player.mp.id, 'Daje vam dokumente.');
  }
  ...
  show(player: Player, type: Document, target?: Player) {
    ...
    switch (type) {
      case 'passport':
        ...
        break;
      case 'licenses':
        this.showPlayerDocs(player, 'licenses', {
          licenses: targetPlayer.licenses,
        });
        break;
      case 'faction':
  ...
  private showPlayerDocs(
    player: Player,
    type: 'passport' | 'licenses',
    data: any
  ) {
