...
class Clothes {
	set = (player: Player, name: ClothesName, item: ClothesItem) => {
		...
		if (gender && player.gender !== gender) {
			hud.showNotification(
				player,
				'error',
				'Ova odeća nije kompatibilna sa vašim polom',
				true
			);
		...
		switch (name) {
			...
			default: {
				if (this.isComponent(name))
					mp.setClothes(components[name], style, color, 2);
	...
	hide(player: Player, name: ClothesName) {
		...
		this.set(player, name, {
			style: this.isComponent(name)
				? empty[gender][components[name]]
				: 255,
			color: 0,
			gender,
		});
	...
	load(player: Player) {
		...
		Object.values(player.equipment).forEach((item) => {
			if (item.data?.style >= 0)
				this.set(player, item.name as any, item.data as any);
		});
	}
	...
	private getComponentData(player: PlayerMp, name: keyof typeof components) {
		return (
			components[name] === 11
				? player.getVariable(name) ??
				  player.getClothes(components[name])
				: player.getClothes(components[name])
		) as ReturnType<typeof player.getClothes>;
	}
	...
	private setJacket(player: Player, data: ClothesItem) {
		...
		if (this.isEmpty(player, 'jacket', data.style)) {
			this.setClientClothes(mp, 'jacket', { style: -1, color: 0 });
			this.setShirt(player, { style: -1, color: 0 });
	...
	private setUndershirt(
		player: Player,
		top: number,
		{ style, color }: ClothesItem
	) {
		const { mp, gender } = player;

		const type: number = tops[gender][top];
		const drawable: number =
			undershirts[gender][style]?.[type] ?? empty[gender][8];
