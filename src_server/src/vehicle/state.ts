...
class VehicleState {
  ...
  toggleTrunk(vehicle: VehicleMp) {
    ...
    if (doorsState?.length < 6) {
      doorsState = [...new Array(6).fill(0)];
    }

    doorsState.forEach((_, index) => {
      if (index === 5) {
        doorsState[index] = trunk ? 0 : 1;
      } else {
        doorsState[index] = doors[index];
      }
    });

    this.setDoors(vehicle, doorsState);
