...
class VehicleTrade {
  ...
  private showMenu(player: Player) {
    ...
    if (!vehicles.length || !target) {
      return hud.showNotification(player, 'error', 'Nemate vozilo', true);
    }
  ...
  private offerToBuy(player: Player, vehicle: string, data: Data) {
    ...
    customer.callEvent('VehicleTrade-ShowMenu', {
      ...data,
  ...
  private async confirm(
    ...
    if (
      !seller.vehicles.includes(vehicleId) ||
      !player.isEnoughVehicleSlots()
    ) {
      return mp.events.reject('Nemate više parking mesta!');
    }

    const vehicle = mp.vehicles.getById(vehicleId);
    const doc = await VehicleModel.findById(vehicleId).select({
      oldOwners: 1,
      owner: 1,
    });
  ...
  private async getData(player: Player, id: string) {
    ...
    return (
      data && {
        model: data.name,
        govNumber: data.govNumber || 'NEREGISTROVANO',
