...
class VehicleShop extends Component<Props, State> {
  ...
  async buy() {
    ...
    try {
      ...
      showNotification(
        'success',
        'Uspešno ste kupili vozilo. Pomoću telefona zatražite mehaničaru da vam dostavi vaše vozilo.'
      );
    } catch (err: any) {
  ...
  render() {
    ...
    return (
      <div className='vehicle-shop'>
        <Hint
          className='vehicle-shop_hint'
          action='exit'
        >
          Kliknite da zatvorite meni
        </Hint>
        ...
        <Color
          current={color}
          select={this.selectColor.bind(this)}
        />
        ...
        <div className='vehicle-shop_main'>
          <span className='vehicle-shop_price'>
            {type === 'vip_shop' ? (
              <Point
                className='vehicle-shop_price'
                amount={prices[selectedVeh]}
              />
            ) : (
          ...
          <div className='vehicle-shop_buttons'>
            <GradientButton
              color='purple'
              onClick={this.startTestDrive.bind(this)}
            >
              Test vožnja
            </GradientButton>

            <GradientButton onClick={this.buy.bind(this)}>Kupi</GradientButton>
          </div>
        </div>
      </div>
    );
