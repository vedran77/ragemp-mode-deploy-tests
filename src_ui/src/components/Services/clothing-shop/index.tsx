...
class ClothingShop extends Component<Props, State> {
  ...
  async setCategory(name: string) {
    const amount: number = await rpc.callClient(
      'ClothingShop-ChangeType',
      name
    );
  ...
  async changeСolor(index: number) {
    ...
    await rpc.callClient('ClothingShop-ChangeItem', [
  ...
  render() {
    const { activeCategory, items, colors, currentItem, currentColor, price } =
      this.state;
        ...
        <div className='clothing-shop_container'>
          <div className='clothing-shop_main'>
            <div className='clothing-shop_color'>
              <h4 className='clothing-shop_color-title'>Model</h4>
            ...
            <div className='clothing-shop_color'>
              ...
              <Selector
                className='clothing-shop_selector'
                value={currentColor}
                items={[...Array(colors).keys()]}
                onChange={this.changeСolor.bind(this)}
              />
            </div>
          </div>
          ...
          <div className='clothing-shop_buttons'>
            ...
            <OutlineButton
              onClick={() => rpc.callClient('ClothingShop-CloseMenu')}
            >
              Zatvori
            </OutlineButton>
          </div>
