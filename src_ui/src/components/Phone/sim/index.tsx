...
export default class Sim extends Component<{}, State> {
  ...
  async buy(num?: string) {
    try {
      const updated: string = await rpc.callServer(
        'Phone-BuyNumber',
        num?.trim()
      );
      ...
      <div className='sim'>
        ...
        {form ? (
          ...
          <form className='sim_form'>
            <input
              type='text'
              value={phoneNumber || 'Nepoznato'}
              readOnly
            />

            <Button onClick={this.openForm.bind(this, 'random')}>Random</Button>
            <Button onClick={this.openForm.bind(this, 'custom')}>Custom</Button>
          </form>
        )}

        <Prices
          random={prices.random}
          custom={prices.custom}
        />
      </div>
    );
  }
