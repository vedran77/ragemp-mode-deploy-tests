...
export default class Workshop extends Component<Props, State> {
  ...
  async getProduct() {
    ...
    try {
      const materials: number = await rpc.callServer(
        'FactionWorkshop-CraftItem',
        [selectedProduct.name, selectedProduct.amount]
      );

      this.setState(() => ({ materials, selectedProduct: undefined }));
      showNotification('success', 'Uspešno napravljeno!');
    } catch (err: any) {
      ...
      <div className='workshop'>
        <div className='workshop_container'>
          <div className='workshop_header'>
            <PrimaryTitle className='workshop_title'>Radionica</PrimaryTitle>
        ...
        <div className='workshop_footer'>
          ...
          <GradientButton onClick={this.getProduct.bind(this)}>
            Napravi
          </GradientButton>
        </div>
      </div>
