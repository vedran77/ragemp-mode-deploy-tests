...
class GangCapture {
	...
	constructor() {
		mp.events.subscribe({
			...
			'GangCapture-ShowCaptureDialog': this.showCaptureDialog.bind(this),
		});
	}
	...
	private decreaseTime() {
		this.capture.time -= 1;
		if (this.capture.time <= 0) return this.stop();
	...
	private showCaptureDialog(show: boolean) {
		mp.events.callBrowser('HUD-SetCapture', [this.capture, show], false);
	}
}
