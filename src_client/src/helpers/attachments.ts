...
class Attachments implements AttachmentsMp {
	...
	private shutdownFor(entity: any) {
		...
		Object.keys(entity.objects).forEach((item) => this.removeFor(entity, parseInt(item, 10)));
	}
	...
	private subscribeToEvents() {
		...
		mp.events.subscribeToData({
			attachments: (entity: PlayerMp | VehicleMp, data: string) => {
				...
				mp.console.logInfo(`Entity in sync: ${JSON.stringify(entity)}`);
				mp.console.logInfo(`Attachments in sync: ${JSON.stringify(attachments)}`);
