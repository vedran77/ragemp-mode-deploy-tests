...
class CharacterCreator {
	...
	constructor() {
		...
		mp.events.subscribe({
			...
			'Character-UpdateInformation': this.updateInformation.bind(this),
	...
	private updateParents(mother: number, father: number, similarity: number, skin: number) {
		this.state.skindata = [mother, father, similarity, skin];

		player.setHeadBlendData(mother, father, 0, mother, father, 0, similarity, skin, 0, true);
	}

	private changeGender(gender: string) {
		this.reset({ gender });

		player.model = mp.game.joaat(gender === 'female' ? 'mp_f_freemode_01' : 'mp_m_freemode_01');
	...
	private updateInformation(data: { firstName: string; lastName: string }) {
	...
	private async create() {
		...
		const charId = await mp.events.callServer('Character-Create', [
			data,
			{
		...
		mp.console.logInfo(`Character created: ${JSON.stringify(data)}`);

		mp.cameras.reset();
		mp.browsers.hidePage();
		this.reset();

		await Auth.onCharacterSelected(charId);
	...
	private showCreator(preview = false) {
