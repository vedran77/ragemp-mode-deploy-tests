...
const controls = {
  default: [
    12, 13, 14, 15, 16, 17, 37, 99, 100, 140, 157, 158, 159, 160, 161, 162, 163,
    164, 165, 261, 262, 263,
  ],
...
class PlayerControls {
  ...
  private onRender() {
    ...
    if (
      this.cuffs ||
      (greenZone.playerWithin && !localPlayer.getVariable('govMember'))
    ) {
    ...
    if (localPlayer.vehicle) {
      controls.vehicle.forEach((item) => this.toggleControl(item, false));
    }
  }
