...
let pointing = {
  ...
  stop: function () {
    if (this.active) {
      ...
      if (!mp.game.invoke('0x84A2DD9AC37C35C1', mp.players.local.handle))
  ...
  getRelativePitch: function () {
    let camRot = mp.cameras.new('gameplay').getRot(2);
  ...
  process: function () {
    if (this.active) {
      ...
      let coords = mp.players.local.getOffsetFromGivenWorldCoords(
      ...
      if (Date.now() - this.lastSent > 100) {
        ...
        mp.events.callRemote('Player-PointUpdate', camPitch, camHeading);
      }
    }
  },
};

mp.events.add('Player-PointUpdate', (id, camPitch, camHeading) => {
