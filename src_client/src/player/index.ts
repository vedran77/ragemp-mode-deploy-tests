...
mp.events.subscribe({
  ...
  'Player-SetWantedLevel': (level: number) => {
    localPlayer.setVariable('wantedLevel', level);
  },
});
...
class Player {
  getName(player: PlayerMp, separator = false) {
    ...
    const isColleague =
      faction && isEqual(faction, player.getVariable('faction'));
    const isFamiliar =
      isColleague ||
      (isFriend && !player.getVariable('inMask')) ||
      localPlayer.getVariable('ADUTY');

    return isFamiliar ? `${player.name.replace('_', ' ')} (${id})` : `${id}`;
  }
}
