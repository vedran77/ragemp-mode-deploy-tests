...
class TargetMenu {
  ...
  constructor() {
    binder.bind('target', 'G', this.toggleMenu.bind(this), null);
  ...
  async toggleMenu() {
    ...
    if (this.show) {
      await mp.events.callBrowser('Target-Reset');
      mp.game.graphics.transitionFromBlurred(0);
    } else {
      ...
      mp.browsers.setHideBind(() => this.toggleMenu());
    }
