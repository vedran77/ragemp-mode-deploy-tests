...
var CasinoPedss = [
	{
		Hash: 0xbc92bed5,
		Pos: new mp.Vector3(1149.391, 269.1934, -51.8409),
		Angle: 47.5,
	},
	{
		Hash: 0x1422d45b,
		Pos: new mp.Vector3(1151.28, 267.33, -51.84),
		Angle: 222.2,
	},
	{
		Hash: 0xbc92bed5,
		Pos: new mp.Vector3(1128.853, 261.8036, -51.041),
		Angle: 314.2,
	},
	{
		Hash: 0x1422d45b,
...
const BlackJackTablesHeading = [-134.69, 45.31, 135.31, 135.31];
...
const s_propName2 = mp.game.joaat('ch_prop_casino_blackjack_01b');
const s_propName = mp.game.joaat('vw_prop_casino_blckjack_01');
let BlackJackTables = [];
let BlackJackTables2 = [];

setTimeout(function () {
	let n = 0;
	for (let tb = 0; tb < 4; tb++) {
		BlackJackTables[tb] = mp.objects.new(
			s_propName,
			new mp.Vector3(BlackJackTablesPos[tb].x, BlackJackTablesPos[tb].y, BlackJackTablesPos[tb].z),
			{
...
mp.game.streaming.requestModel(mp.game.joaat('ch_prop_casino_blackjack_01b'));
...
mp.events.add('client_bj_give_card', function (table, play, num, card) {
	...
	setTimeout(() => {
		CardObjects[table][play][num] = mp.objects.new(
			mp.game.joaat(`${CasinoCards[card]}`),
			new mp.Vector3(tableCords.x, tableCords.y, tableCords.z),
			{
		...
		let cardPos = mp.game.object.getObjectOffsetFromCoords(
			tableCords.x,
			tableCords.y,
			tableCords.z,
			BlackJackTablesHeading[table],
			cardOff.x,
			cardOff.y,
			cardOff.z
		);
		...
		CardObjects[table][play][num].attachTo(
			CasinoPedssID[table].handle,
			CasinoPedssID[table].getBoneIndex(28422),
			0.0,
			0.0,
			0.0,
			0.0,
		...
		CasinoPedssID[table].taskPlayAnim(
	...
	setTimeout(() => {
		...
		let newCardPos = mp.game.object.getObjectOffsetFromCoords(
			tableCords.x,
			tableCords.y,
			tableCords.z,
			BlackJackTablesHeading[table],
		...
		mp.game.invoke(
			'0x239A3351AC1DA385',
			CardObjects[table][play][num].handle,
			newCardPos.x,
			newCardPos.y,
			newCardPos.z,
			false,
		...
		let rotatet =
			play == 4 && num == 1
				? new mp.Vector3(0, 180, BlackJackTablesHeading[table])
				: new mp.Vector3(0, 0, BlackJackTablesHeading[table] + BlackJackTableCardsRotation[play][num]);
...
mp.events.add('flip_card', function (table) {
	...
	setTimeout(() => {
		if (CardObjects[table][4][1] != undefined)
			CardObjects[table][4][1].attachTo(
				CasinoPedssID[table].handle,
				CasinoPedssID[table].getBoneIndex(28422),
				0.0,
				0.0,
...
mp.events.add('clean_cards', function (table, play) {
	if (play == 4)
		...
		CasinoPedssID[table].taskPlayAnim(
			'anim_casino_b@amb@casino@games@blackjack@dealer',
			`retrieve_cards_player_0${play + 1}`,
			3.0,
			1.0,
			-1,
			2,
	...
	setTimeout(() => {
		if (CardObjects[table][play][0] != undefined || CardObjects[table][play][0] != null)
			CardObjects[table][play][0].attachTo(
				CasinoPedssID[table].handle,
				CasinoPedssID[table].getBoneIndex(28422),
				0.0,
				0.0,
		...
		for (let i = 0; i < betBlackjack[table].length; i++) {
			if (betBlackjack[table][i] != null) betBlackjack[table][i].destroy();
		}
...
mp.events.add('stand_or_hit', function (table, seat) {
	CasinoPedssID[table].taskPlayAnim(
		'anim_casino_b@amb@casino@games@blackjack@dealer',
		`dealer_focus_player_0${seat + 1}_idle`,
		3.0,
		1.0,
		-1,
		2,
...
mp.events.add('request_card', function (playerRemoteId) {
	const handle = mp.players.atRemoteId(playerRemoteId).handle;
...
mp.events.add('decline_card', function (playerRemoteId) {
	const handle = mp.players.atRemoteId(playerRemoteId).handle;
	const player = mp.players.atHandle(handle);
	player.taskPlayAnim(
		'anim_casino_b@amb@casino@games@blackjack@player',
...
mp.events.add('exit_table', function (playerRemoteId) {
	const handle = mp.players.atRemoteId(playerRemoteId).handle;
	const player = mp.players.atHandle(handle);
...
mp.events.add('bet_blackjack', function (table, seat, playerRemoteId) {
	const handle = mp.players.atRemoteId(playerRemoteId).handle;
	const player = mp.players.atHandle(handle);
	player.taskPlayAnim(
		'anim_casino_b@amb@casino@games@blackjack@player',
	...
	setTimeout(() => {
		betBlackjack[table].push(
			mp.objects.new(
				mp.game.joaat(`vw_prop_chip_100dollar_x1`),
				new mp.Vector3(newCardPos.x, newCardPos.y, newCardPos.z),
				{
					rotation: new mp.Vector3(0, 0, 0),
					alpha: 255,
					dimension: 0,
...
mp.events.add('show_blackjack', (toggle) => {
	if (toggle) {
		mp.browsers.showPage('blackjack');
...
mp.events.add('seat_to_blackjack_table', function (table, seat, playerRemoteId) {
	const handle = mp.players.atRemoteId(playerRemoteId).handle;
	const player = mp.players.atHandle(handle);
...
mp.keys.bind(0x26, false, function () {
	if (!mp.players.local.getVariable('canDoBets')) {
		rpc.callServer('blackJackBetUp');
	}
});
mp.keys.bind(0x28, false, function () {
	if (!mp.players.local.getVariable('canDoBets')) {
		rpc.callServer('blackJackBetDown');
	}
});

mp.keys.bind(0x0d, false, function () {
	if (!mp.players.local.getVariable('canDoBets')) {
		rpc.callServer('blackJackSetBet');
	}
});

mp.keys.bind(0x20, false, function () {
	if (!mp.players.local.getVariable('canDoBets')) {
		rpc.callServer('blackJackHit');
	}
});
