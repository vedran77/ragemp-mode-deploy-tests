...
class CruiseControl {
  ...
  private toggle() {
    ...
    if (!this.isActivated) {
      ...
      mp.game.ui.notifications.show('info', 'Tempomat je aktiviran');
    } else {
      this.maxSpeed = 0;

      vehicle.setMaxSpeed(1000.0);

      mp.game.ui.notifications.show('info', 'Tempomat je deaktiviran');
    }
  }
  ...
  private bindKeys() {
    binder.bind('cruise', 'Caps lock', this.toggle.bind(this));
  }
}
