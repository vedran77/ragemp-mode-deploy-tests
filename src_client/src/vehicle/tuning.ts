...
class VehicleTuning {
	...
	setTyres(vehicle: VehicleMp, model: number) {
		mp.game.invoke('0x6AF0636DDEDCB6DD', vehicle.handle, 23, vehicle.getMod(23), model > -1);
	}
	...
	set(vehicle: VehicleMp, data: TuningData) {
		...
		vehicle.setLivery(data.livery);

		for (let i = 0; i <= 16; i++) vehicle.setExtra(i, data.extras.includes(i) ? 0 : 1);
	}
}
