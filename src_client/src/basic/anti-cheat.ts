...
class AntiCheat {
  ...
  init() {
    ...
    setInterval(() => {
      if (!this.active || player.getVariable('adminLvl'))
        return this.savePosition();

      if (!this.isNormalDiffOfPosition(40)) this.report('Teleport');

      if (player.vehicle) {
        if (!this.isAllowedGroundDistOfVehicle(25)) this.report('Vehicle Fly');
        if (this.checkVehicleMaxSpeed()) this.report('Speed Hack');
      }
  ...
  private isNormalDiffOfPosition(limit = 30) {
    ...
    return (
      !this.isWalking() ||
      (Math.abs(diff.x) <= limit && Math.abs(diff.y) <= limit)
    );
  }

  private checkVehicleMaxSpeed() {
    const { vehicle } = player;
    const maxSpeed = mp.game.vehicle.getVehicleModelMaxSpeed(
      player.vehicle.model
    );

    return (
      vehicle &&
      !this.isAirVehicle(vehicle) &&
      vehicle.getSpeed() > maxSpeed * 2
    );
  }
