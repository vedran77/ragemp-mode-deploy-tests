...
class Dialog {
	...
	private isPlayerAuthorized(pedIndex) {
		return (
			peds?.[pedIndex]?.authorities &&
			!peds?.[pedIndex].authorities.includes(
	...
	private show({ index }: Data) {
		if (this.isPlayerAuthorized(index) || player.vehicle) return;
	...
	private onEnterPedZone({ index, ped }: Data) {
		...
		if (this.isPlayerAuthorized(index)) return;
