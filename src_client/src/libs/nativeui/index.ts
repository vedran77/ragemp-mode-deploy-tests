...
export default class NativeUI {
	...
	public SelectItem() {
		...
		if (
			this.MenuItems[this.CurrentSelection] instanceof UIMenuCheckboxItem
		) {
	...
	public ProcessMouse() {
		if (
			...
			this.MenuItems.filter((i) => i.Hovered).forEach(
				(i) => (i.Hovered = false)
			);
			return;
		}
		...
		if (
			...
			this.IsMouseInBounds(
				new Point(
					this.GetScreenResolutionMantainRatio().Width - 30.0,
					0
				),
				new Size(30, 1080)
			) &&
		...
		for (let i = this._minItem; i <= limit; i++) {
			...
			let ypos =
				this.offset.Y + 144 - 37 + this.extraOffset + counter * 38;
	...
	private render() {
		...
		if (this.MenuItems.length <= this.MaxItemsOnScreen + 1) {
			...
			this._extraRectangleDown.size = new Size(
				431 + this.WidthOffset,
				18
			);
