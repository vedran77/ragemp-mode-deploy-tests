...
class CbPoint {
	...
	getCenterPoint(pos, xDeltaOverYDelta, xDelta, yDelta, distance) {
		if (xDeltaOverYDelta) {
			return xDelta < 0
				? { x: pos.x + distance / 2, y: pos.y }
				: { x: pos.x - distance / 2, y: pos.y };
		}
		return yDelta < 0
			? { x: pos.x, y: pos.y + distance / 2 }
			: { x: pos.x, y: pos.y - distance / 2 };
	}
	...
	drawTailHistoryAndCheckCollisions(status) {
		...
		for (let i = 0; i < historyClone.length; i++) {
			...
			else historyNextPoint = historyClone[i + 1];
	...
	addToTailHistory(directionChangePoint) {
		if (
			!this.history.some(
				(e) =>
					e.x === directionChangePoint.x &&
					e.y === directionChangePoint.y
			)
		) {
	...
	getPointInputFromPlayer() {
		...
		if (
			...
			this.isOppositeOfCurrentDirection(newDirection)
		) {
			return;
		}
		this.lastDirection = newDirection;
		this.addToTailHistory(lastPos);
		mp.game1.audio.playSoundFrontend(
			-1,
			'Click',
	...
	setPointStartPosition() {
		const magnitude =
			cbGenericPorts.startPortHeading === 0 ||
			cbGenericPorts.startPortHeading === 180
				? 0.0144
				: 0.021;
		this.position = cbHelper.getOffsetPosition(
			cbGenericPorts.startPortPos,
			magnitude,
			cbGenericPorts.startPortHeading,
			1
		);
	}
