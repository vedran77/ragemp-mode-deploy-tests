...
const { cbGenericPorts } = require('./CbGenericPorts');
const { cbHelper } = require('./CbHelper');
const { cbMapBoundaries } = require('./CbMapBoundaries');
const { cbPoint } = require('./CbPoint');
const { Scaleform } = require('./Scaleform');

class CircuitBreaker {
	constructor(lives, difficulty, levels, events, onDestroy) {
		this.lives = lives;
		...
		this.currentPointSpeed = this.getPointSpeedFromDifficulty(
			this.difficulty
		);
		this.disconnectChance = this.getDisconnectChanceFromDifficulty(
			this.difficulty
		);
		this.disconnectCheckRateMs =
			this.getDisconnectCheckRateMsFromDifficulty(this.difficulty);
		this.init();
	}
	getLevel() {
		var _a;
		if (this.levelsToComplete.length === 0) return 1;
		return (_a = this.levelsToComplete.shift()) !== null && _a !== void 0
			? _a
			: 1;
	}
	fillLevels(count) {
		if (count < 1) return;
		this.levelsToComplete = new Array(count)
			.fill(1)
			.map(() => this.getRandomLevel());
	}
	getRandom(min, max) {
	...
	successLogic() {
		...
		mp.events.callServer(this.events.onWin);
	}
	restartSameLevel() {
		this.status = 'DEATH';
		...
		this.nextTimeCheckDisconnect =
			this.startTime + this.disconnectCheckRateMs;
	}
	failureLogic() {
		this.livesLeft--;
		if (this.livesLeft > 0) {
		...
		if (!cbPoint.isVisible) {
			...
			if (now >= this.endTime) {
				...
				mp.events.callServer(this.events.onLose);
			}
		}
	}
	deathLogic() {
	...
	disconnectLogic() {
		if (this.reconnectIn === 0) {
			...
			this.showDisplayScaleform(
				'CONNECTION LOST',
				'Reconnecting...',
				cbHelper.RED_COLOUR.r,
				cbHelper.RED_COLOUR.g,
				cbHelper.RED_COLOUR.b,
				false
			);
	...
	resetSounds() {
		...
		mp.game.audio.stopSound(parseInt(this.soundId));
		mp.game.audio.releaseSoundId(parseInt(this.soundId));
		this.soundId = -1;
	}
	playStartSound() {
		mp.game.audio.playSoundFrontend(
	...
	showSuccessScreenAndPlaySound() {
		...
		this.showDisplayScaleform(
			'CIRCUIT COMPLETE',
			'Decryption Execution x86 Tunneling',
			cbHelper.GREEN_COLOUR.r,
			cbHelper.GREEN_COLOUR.g,
			cbHelper.GREEN_COLOUR.b,
			true
		);
	}
	...
	drawMapSprite(currentMap) {
		mp.game.graphics.drawSprite(
			currentMap > 3 ? 'MPCircuitHack3' : 'MPCircuitHack2',
			`cblevel${this.level}`,
			0.5,
			0.5,
			1,
	...
	drawPointAndPortSprites() {
		...
		const collisionHit = cbPoint.drawTailHistoryAndCheckCollisions(
			this.status
		);
	...
	isPointOutOfBounds(polybounds, mapBounds) {
		const headPts = this.getPointMaxPoints(
			cbPoint.position,
			cbPoint.pointHeadSize + -0.375 * cbPoint.pointHeadSize
		);
